  ...
  try {
    ...
    if (event.httpMethod === "POST") {
      ...
      if (body.rolloverOpen) {
        ...
        // переносим незакрытые задачи на новый день, если они ещё на более раннем дне
        // day хранится как YYYY-MM-DD, поэтому строковое сравнение корректно
        ...
        return json(200, { ok: true, rolledTo: targetDay });
      }
      ...
      if (body.upsert && body.task) {
        ...
        const row = {
          ...
          creator_profile_id: String(t.creator_profile_id || "").trim(),
        ...
        if (!row.owner_profile_id || !row.creator_profile_id || !row.text || !row.day) {
          return json(400, { error: "owner_profile_id, creator_profile_id, text and day are required" });
        }
